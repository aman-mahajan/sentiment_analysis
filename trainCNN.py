...
print "loading data..."
x = cPickle.load(open("rt-train-val-test.pickle", "rb"))
revs, W, word_idx_map, vocab = x[0], x[1], x[2], x[3]
print "data loaded!"


datasets = make_idx_data(revs, word_idx_map, kernel_size=5)
...
train_Y = np.zeros((N, 5), dtype=np.int)
for i in xrange(N):
...
val_Y = np.zeros((Nv, 5), dtype=np.int)
for i in xrange(Nv):
    for j in xrange(conv_input_height):
...
for i in xrange(N_epoch):
    model.fit(train_X, train_Y, batch_size=100, nb_epoch=1, verbose=1)
    output = model.predict_proba(val_X, batch_size=10, verbose=1)
    # find validation accuracy using the best threshold value t
    vacc = sum(output.argmax(axis==1) == val_Y.argmax(axis=1))*1.0/val_Y.shape[0]
    # find validation AUC
    ...
    epoch += 1
    model.save_weights('cnn_5_class'+str(i)+'.model')
...
d = pd.DataFrame({'PhraseId': data['PhraseId'], 'Sentiment': p[:,0]})
